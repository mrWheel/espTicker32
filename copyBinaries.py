...
def handle_firmware(source, target, env):
    ...
    if program_name and version_str:
        print(f"DEBUG: Managing binaries with program_name: {program_name}, version: {version_str}, postfix: None")
        manage_binaries(firmware_bin, program_name, version_str, None)
    else:
        print(f"DEBUG: Skipping binary management due to missing program_name or version_str")
